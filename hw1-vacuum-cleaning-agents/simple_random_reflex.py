...
class SimpleStochasticAgent:
	...
	def rule_match(self, state):
		...
		if state == "Dirty":
			...
			action = np.random.choice(["GoHead", "TurnRigt", "TurnLeft"], 1, p=[0.4, 0.3, 0.3])
			#action = np.random.choice(["GoHead", "TurnRigt", "TurnLeft"], 1, p=[0.7, 0.2, 0.1])
		elif state == "Blocked":
			action = np.random.choice(["TurnRigt", "TurnLeft"], 1, p=[0.5, 0.5])
			#action = np.random.choice(["TurnRigt", "TurnLeft"], 1, p=[0.7, 0.3])
		else: # BackHome
			action = "Off"
		return action
