...
class ModelDeterministicAgent:
	...
	def update_state(self, state):
		...
		if self.memory == [0,0,0] and state == "Free":
			...
			new_state = "RevertL"
		elif self.memory == [1,0,1] and state == "Blocked":
			new_state = "RevertL"
			...
			new_state = "RevertR"
		elif self.memory == [0,1,0] and state == "Blocked":
			new_state = "RevertR"
	...
	def update_memory(self, action):
		...
		if self.memory == [0,0,0] and action == "TurnRight":
			...
			new_memory = [1,1,0]
		elif self.memory == [1,0,0] and action == "TurnRight":
			new_memory = [1,0,1]
		elif self.memory == [1,1,0] and action == "TurnRight":			
			new_memory = [1,1,1]
		elif self.memory == [1,0,1] and action == "TurnLeft":			
			new_memory = [1,1,1]
		elif self.memory == [1,1,1] and action == "TurnLeft":
			new_memory = [0,1,1]
		elif self.memory == [0,1,1] and action == "GoHead":
			new_memory = [0,0,1]
		elif self.memory == [0,1,1] and action == "TurnRight":
			new_memory = [0,1,0]
		elif self.memory == [0,0,1] and action == "TurnLeft":
			new_memory = [0,0,0]
		elif self.memory == [0,1,0] and action == "TurnRight":			
			new_memory = [0,0,0]		
		else:
	...
	def update_position(self, action, location, direction):
		...
		# direction = 0-Head, 1-Right, 2-Down, 3-Left
		if action == "GoHead":
			if direction == 0:
				location[1] += 1
			elif direction == 1:
